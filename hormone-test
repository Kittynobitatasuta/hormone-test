...
with col_in:
    ...
    psa = st.number_input("PSA (ng/mL)", min_value=0.0, max_value=50.0, value=0.0, step=0.01)

    age = st.number_input("อายุคนไข้ (ปี)", min_value=18, max_value=95, value=40)
    on_trt = st.checkbox("คนไข้กำลังรับ TRT อยู่")

    analyze = st.button("🔍 แปลผล", use_container_width=True)
...
with col_out:
    ...
    if not analyze:
        ...
        if not any_input:
            ...
            # =============================================
            # BLOCK D: PSA → คัดกรองมะเร็งต่อมลูกหมาก
            # =============================================
            if psa > 0:
